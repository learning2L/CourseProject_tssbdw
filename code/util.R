...
#' @param ... additional args for logprob              # 这里的省略号是 logprob 需要的传入参数（可选）
...
# 注意：[Neal2003] 中提到，为避免
  ...
  DirectionSlice <- function(direction, initX) {
    ...
    DirLogProb <- function(z) {  # 不需要传入 direction 也没关系（原理：应该是找不到此参数就去外一层函数中寻找）
      return(logprob(direction*z + initX, ...))  # 省略号是出现在 logprob 的调用中的，也说明是它的传入参数
    }

    llhS <- log(runif(1)) + DirLogProb(0)  # log(runif(1)) 等价于 rexp(1)！！！
    upper <- sigma * runif(1)
    lower <- upper - sigma
    lowerStepsOut <- 0
    upperStepsOut <- 0
